(function () {
  ...
  function renderAlarmes(alarmes) {
  ...
  async   // Funções de busca com debounce
  const debouncedSearch = debounce(async (term) => {
    try {
      await loadAlarmeOptions(term);
    } catch (error) {
      errorHandler.handle(error, 'Debounced search failed', 'warning');
    }
  }, 300);
  ...
  const debouncedFetchDados = debounce(async () => {
    ...
    if (!select) return;
    select.innerHTML = '<option value="">Carregando alarmes...</option>';
    if (selectOs) {
      selectOs.innerHTML = '<option value="">Carregando alarmes...</option>';
    }
    ...
    try {
      ...
      if (state.alarmeSelecionada) {
        ...
        ensureSelectOption(equipamentoSelect, String(state.alarmeSelecionada), `Alarme ${state.alarmeSelecionada}`);
        ensureSelectOption(equipamentoOsSelect, String(state.alarmeSelecionada), `Alarme ${state.alarmeSelecionada}`);
      }
    } catch (e) {
      console.error('Erro ao carregar alarmes:', e);
      select.innerHTML = '<option value="">Erro ao carregar alarmes</option>';
      if (selectOs) {
        selectOs.innerHTML = '<option value="">Erro ao carregar alarmes</option>';
      }
    }
  }
  ...
  async function fetchDados() {
    ...
    const selectedAlarme = document.getElementById('equipamento_id')?.value || '';
    ...
    const alarmeToSet = state.alarmeSelecionada || selectedAlarme;
    if (alarmeToSet) {
      document.getElementById('equipamento_id').value = alarmeToSet;
      document.getElementById('equipamento_id_os').value = alarmeToSet;
    }
  ...
  async function exportarCsvHistorico() {
    ...
    let csv = 'Data/Hora;OS;Alarme;IP;Série;Modelo;Local Serviço;Endereço Serviço;Procedimento;Status;Técnico;Descrição;Peças;Usuário\n';
    rows.forEach((item) => {
      csv += [
        csvEscape(toBrDate(item.data_execucao || item.data_hora || item.created_at)),
        csvEscape(item.numero_os || ''),
        csvEscape(item.alarme_nome || ''),
        csvEscape(item.ip || ''),
  ...
  async function exportarPdfHistorico() {
    ...
    const htmlRows = rows.map((item) => `
      <tr>
        ...
        <td>${escapeHtml(item.numero_os || '-')}</td>
        <td>${escapeHtml(item.alarme_nome || '-')}</td>
        <td>${escapeHtml(item.ip || '-')}</td>
...
<body>
  ...
  <table>
    <thead>
      <tr>
        <th>Data/Hora</th><th>OS</th><th>Alarme</th><th>IP</th><th>Série</th>
  ...
  <script>window.onload=function(){window.print();};</script>
    ...
    if (!printWindow) return showMessage('danger', 'Não foi possível abrir a janela de impressão. Verifique o bloqueador de pop-up.');
  ...
  async function salvarManutencao(event) {
    ...
    try {
      ...
      if (manutencaoId) {
        ...
        if (listEl) {
          listEl.setAttribute('data-manutencao-alarme-id', manutencaoId);
      ...
      state.alarmeSelecionada = payload.alarme_id;
  ...
  function bindEvents() {
    ...
    document.getElementById('equipamento_id_os')?.addEventListener('change', (event) => {
      ...
      const localEl = document.getElementById('local_servico_os');
      if (localEl) localEl.value = alarme?.local_nome || '';
      if (id) {
    ...
    const btnBuscar = document.getElementById('btnBuscarAlarmeLista');
    const filtro = document.getElementById('filtroAlarme');
    if (btnBuscar && filtro) {
      btnBuscar.addEventListener('click', async () => {
        state.alarmeBusca = String(filtro.value || '').trim();
        await loadAlarmeOptions(state.alarmeBusca);
      });
      filtro.addEventListener('keydown', async (event) => {
    ...
    document.getElementById('btnLimparSelecaoOs')?.addEventListener('click', clearOrdemSelecao);
    ...
    document.getElementById('historicoPerPage')?.addEventListener('change', async (event) => {
      state.historicoPerPage = Number(event.target.value || 20);
      state.historicoPage = 1;
      await fetchDados();
    });
    ...
    document.getElementById('btnExportarCsvHistorico')?.addEventListener('click', async () => {
      try { await exportarCsvHistorico(); } catch (error) { showMessage('danger', error.message || 'Falha ao exportar CSV.'); }
    });
    document.getElementById('btnExportarRapidoHistorico')?.addEventListener('click', async () => {
      try { await exportarPdfHistorico(); } catch (error) { showMessage('danger', error.message || 'Falha ao gerar relatório.'); }
    });
    document.getElementById('btnExportarPdfHistorico')?.addEventListener('click', async () => {
      try { await exportarPdfHistorico(); } catch (error) { showMessage('danger', error.message || 'Falha ao exportar PDF.'); }
    });
  ...
  async function init() {
    ...
    try {
      ...
      showMessage('danger', error.message || 'Falha ao carregar dados de manutenção.');
