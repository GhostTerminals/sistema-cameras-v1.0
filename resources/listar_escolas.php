...
$where = "WHERE l.nome LIKE '%ESCOLA%'";
$params = [];
if ($search !== '') {
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-success text-white">
            <div class="d-flex justify-content-between align-items-center">
                <h4 class="mb-0"><i class="fas fa-school me-2"></i>Lista de Escolas</h4>
        ...
        <div class="card-body">
            <form method="get" class="row g-2 mb-3">
                <input type="hidden" name="page" value="listar_escolas">
                <div class="col-md-4">
                ...
                <div class="col-md-2">
                    <a href="?page=listar_escolas" class="btn btn-outline-secondary btn-sm w-100">
                        <i class="fas fa-times me-1"></i>Limpar
                    </a>
                </div>
            </form>

            <p class="text-muted small">Total: <strong><?= $total ?></strong> escolas encontradas</p>
            ...
            <nav>
                <ul class="pagination pagination-sm justify-content-center">
                    <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=listar_escolas&page_num=<?= $page - 1 ?>&busca=<?= urlencode($search) ?>">Anterior</a>
                    </li>
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                        <a class="page-link" href="?page=listar_escolas&page_num=<?= $i ?>&busca=<?= urlencode($search) ?>"><?= $i ?></a>
                    </li>
                    <?php endfor; ?>
                    <li class="page-item <?= $page >= $totalPages ? 'disabled' : '' ?>">
                        <a class="page-link" href="?page=listar_escolas&page_num=<?= $page + 1 ?>&busca=<?= urlencode($search) ?>">Próximo</a>
                    </li>
                </ul>
            </nav>
            <?php endif; ?>
