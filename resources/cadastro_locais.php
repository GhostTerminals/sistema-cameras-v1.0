...
// PRG: flash data from session (cleared after redirect)
$formData = $_SESSION['form_data_cadastro_local'] ?? [];
unset($_SESSION['form_data_cadastro_local']);

$mensagem_erro = $_SESSION['mensagem_erro_cadastro_local'] ?? '';
$mensagem_sucesso = $_SESSION['mensagem_sucesso_cadastro_local'] ?? '';
unset($_SESSION['mensagem_erro_cadastro_local'], $_SESSION['mensagem_sucesso_cadastro_local']);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    $csrfToken = $_POST['csrf_token'] ?? null;
    if (!validateCsrfToken($csrfToken)) {
        $_SESSION['mensagem_erro_cadastro_local'] = 'Falha de validacao CSRF.';
        header('Location: index.php?page=cadastro_locais');
        exit;
    }
    ...
    if (empty($nome)) {
        ...
        $sql = "INSERT INTO locais (tipo_local_id, nome, regiao_id, secretaria_id, horario_funcionamento, tem_alarme, cep, uf, cidade, bairro, classificacao_endereco_id, logradouro, numero, descricao_posicao, latitude, longitude)
                VALUES (:tipo_local_id, :nome, :regiao_id, :secretaria_id, :horario_funcionamento, :tem_alarme, :cep, :uf, :cidade, :bairro, :classificacao_endereco_id, :logradouro, :numero, :descricao_posicao, :latitude, :longitude)";
        $params = [
        ...
        if ($result['status'] === 'success') {
            ...
            auditEvent($db, 'locais', $newId, 'INSERT', null, [
                'id' => $newId,
                'nome' => $nome,
            ...
            $_SESSION['sucesso'] = 'Local cadastrado com sucesso.';
            header('Location: index.php?page=listar_locais');
            exit;
        } else {
            $mensagem_erro = 'Erro ao cadastrar local.';
        }
    }

    if ($mensagem_erro) {
        $_SESSION['mensagem_erro_cadastro_local'] = $mensagem_erro;
        $_SESSION['form_data_cadastro_local'] = array_intersect_key($_POST, array_flip([
            'tipo_local_id', 'nome', 'regiao_id', 'secretaria_id',
            'horario_funcionamento', 'tem_alarme', 'cep', 'uf',
            'cidade', 'bairro', 'classificacao_endereco_id',
            'logradouro', 'numero', 'descricao_posicao',
            'latitude', 'longitude',
        ]));
        header('Location: index.php?page=cadastro_locais');
        exit;
...
// Helper: get form value from flash or empty
function fd(string $key): string {
    global $formData;
        ...
        <div class="col-xl-10 col-lg-12">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                    <h4 class="mb-0"><i class="fas fa-map-marker-alt me-2"></i>Cadastro de Local</h4>
                    <a href="?page=listar_locais" class="btn btn-outline-light btn-sm">
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-2">
                                ...
                                <select class="form-select form-select-sm" id="tipo_local_id" name="tipo_local_id" required>
                                    ...
                                    <option value="<?= (int)$t->id ?>" <?= (isset($formData['tipo_local_id']) && (int)$formData['tipo_local_id'] === (int)$t->id) ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-select form-select-sm" id="regiao_id" name="regiao_id">
                                    ...
                                    <option value="<?= (int)$r->id ?>" <?= (isset($formData['regiao_id']) && (int)$formData['regiao_id'] === (int)$r->id) ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" id="tem_alarme" name="tem_alarme" value="1"
                                        <?= (isset($formData['tem_alarme'])) ? 'checked' : '' ?>>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select form-select-sm" id="secretaria_id" name="secretaria_id">
                                    ...
                                    <option value="<?= (int)$s->id ?>" <?= (isset($formData['secretaria_id']) && (int)$formData['secretaria_id'] === (int)$s->id) ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control form-control-sm" id="horario_funcionamento" name="horario_funcionamento"
                                    value="<?= fd('horario_funcionamento') ?>"
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-select form-select-sm" id="uf" name="uf">
                                    ...
                                    <option value="<?= $uf ?>" <?= (isset($formData['uf']) && $formData['uf'] === $uf) ? 'selected' : '' ?>><?= $uf ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-select form-select-sm" id="classificacao_endereco_id" name="classificacao_endereco_id">
                                    ...
                                    <option value="<?= (int)$c->id ?>" <?= (isset($formData['classificacao_endereco_id']) && (int)$formData['classificacao_endereco_id'] === (int)$c->id) ? 'selected' : '' ?>>
